...
public class JobMain {
	...
	public void deleteGujikja(String userid) {
		...
		/*
		 Gujikja.count 가 3인 배열의 방번호가 0 1 2 
		 							  
		 0번째 방이 삭제된다라면 deleteIndex는 0이 되고 방을 옮기는 2번의 작업이 일어나야한다.
		 
		 2번째 방이 삭제된더라면 deleteIndex는 2이되고 방을 옮기는 null번의 작업만 해야한다.
		
		 Gujikja.count 가 5인 배열의 방번호가 0 1 2 3 4 일때  0번째 방이 삭된다라면 deleteIndex는 0이 되고 방을 옮기는  4번의 작업이 일어나야 한다.
		 
		 2번째 방이 삭제된더라면 deleteIndex는 2이되고 방을 옮기는 2번의 작업이 일어나야 한다.
		 3번째 방이 삭제된더라면 deleteIndex는 3이되고 방을 옮기는 1번의 작업이 일어나야 한다.
		 4번째 방이 삭제된더라면 deleteIndex는 4이되고 4번째 방은 null 이 들어오는 작업을 해야한다.
		 */
	...
	public void deleteAdmin (Scanner sc) {
		System.out.print("삭제할 ID 검색");	
		int deleteId = sc.nextInt();
		
		for(int i=0;i<Gujikja.count;i++) {
			if(gujikjaArr[i].userid.equals(deleteId)) {			
				i=deleteId;
			}
		}
		
		if(deleteId==Gujikja.count-1) {
			gujikjaArr[deleteId]=null;
		}
		else {
			for(int i=0,j=deleteId; i<Gujikja.count-1-j;i++,j++) {
				gujikjaArr[j] = gujikjaArr[j+1];
				gujikjaArr[j+1] = null;
	...
	//*** 관리자 메뉴 메뉴번호 3. 특정구직자 삭제해주는 메소드 생성 ***
	
	public void deleteSomeGujikja(Scanner sc) {
		System.out.print("▷ 삭제할 구직자 ID 검색");
		String userid = sc.nextLine();
		boolean isExists = false;
		for(int i=0;i<Gujikja.count;i++) {
			if(gujikjaArr[i].userid.equals(userid)) {
				isExists = true;//동일한 아이디 존재 시 true
				break;
			}
			
		}
		if(isExists)
			deleteGujikja(userid);
		else
					...
					if(loginGujikja != null) { // 로그인 성공 했으면
						do { // 구직자 메뉴 보여주기 
							...
							switch (gujikjaMenuno) {
								...
								case "7":  // 회원탈퇴
									jobapp.deleteGujikja(loginGujikja.userid);
						...
						menuNo =sc.nextInt();
							...
							switch (menuNo) {
								case 1:
									...
									break;
								case 2:	System.out.println("2. 모든 구인회사 정보 출력"); break;
								case 3:	System.out.println("3. 특정 구직자 삭제 ");
